...
import { getAdmins, getMembers, getStaffs, getUserList } from "../../services/UserService";
...
const DashboardContent = () => {
  ...
  const data = [
    ...
    {id:"3", title: "Total Member", value: member?.length  || 0, type: "estate"},
    {id:"4", title: "Total User", value:admin?.length + staff?.length + member?.length  || 0, type: "estate"},
  ];
  ...
  return (
    <>
      ...
      <GraphDashboard linegraph={linegraph} piegraph={[admin?.length, staff?.length, member?.length]}/>
    </>
  );
};
